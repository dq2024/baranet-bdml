...
__global__ void op_elemwise_unary_kernel(OpFunc f, Tensor<T> t, Tensor<T> out)
{
  //Lab-1: add your code here
    ...
    if (!IndexOutofBound(t, i, j) && !IndexOutofBound(out, i, j)){
...
void op_elemwise_unary_gpu(OpFunc f, const Tensor<T> &t, Tensor<T> &out)
{
  ...
  dim3 block_size(ELEMWISE_BLOCK_DIM, ELEMWISE_BLOCK_DIM);
  dim3 grid_size((t.w + block_size.x - 1) / block_size.x, (t.h + block_size.y - 1) / block_size.y);
  op_elemwise_unary_kernel<<<grid_size, block_size>>>(f, t, out);
}
...
__global__ void op_elemwise_binary_w_bcast_kernel(OpFunc f, Tensor<AT> in1, Tensor<BT> in2, Tensor<OutT> out)
{
  //Lab-1: add your code here
    ...
    if (!IndexOutofBound(in1, i, j) && !IndexOutofBound(out, i, j)){
        AT a = Index(in1, i, j);
        BT b;
...
void op_elemwise_binary_w_bcast_gpu(OpFunc f, const Tensor<AT> &in1, const Tensor<BT> &in2, Tensor<OutT> &out)
{
  ...
  dim3 block_size(ELEMWISE_BLOCK_DIM, ELEMWISE_BLOCK_DIM);
  dim3 grid_size((in1.w + block_size.x - 1) / block_size.x, (in1.h + block_size.y - 1) / block_size.y);
  op_elemwise_binary_w_bcast_kernel<<<grid_size, block_size>>>(f, in1, in2, out);
}
